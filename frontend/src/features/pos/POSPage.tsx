...
export const POSPage: React.FC = () => {
  ...
  const fetchMedicines = async () => {
    try {
      ...
      if (wailsApp && typeof wailsApp.ListMedicines === 'function') {
        const data = await wailsApp.ListMedicines(search, category === 'All' ? '' : category, false);
        setMedicines(data || []);
      } else {
        setMedicines([
  ...
  const handleUpdateQty = (medId: number, delta: number) => {
    setCart(prev => prev.map(item => {
      if (item.medicine.id === medId) {
        ...
        if (newQty > item.medicine.current_stock) return item;
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      if (wailsApp && typeof wailsApp.ProcessSale === 'function') {
        const sale = await wailsApp.ProcessSale(
          user?.id || 1,
      ...
      setError(err?.message || 'Checkout failed. Please check stock availability.');
    } finally {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: '24px', padding: '24px', maxWidth: '1600px', margin: '0 auto', height: 'calc(100vh - 48px)' }}>
            ...
            return (
              <div
                ...
                style={{
                  backgroundColor: '#fff',
                  borderRadius: '12px',
                  border: inCart ? '2px solid var(--color-primary-teal)' : '1px solid var(--color-border-subtle)',
                  padding: '16px',
                  ...
                  boxShadow: inCart ? '0 4px 12px rgba(26, 157, 139, 0.15)' : '0 2px 6px rgba(0,0,0,0.02)'
                }}
              >
                <div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '6px' }}>
                    ...
                    {inCart && (
                      <span style={{ backgroundColor: 'var(--color-primary-teal)', color: '#fff', borderRadius: '50%', width: '20px', height: '20px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '11px', fontWeight: 700 }}>
                        {inCart.quantity}
                      </span>
                    )}
                  </div>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid var(--color-border-subtle)', paddingTop: '10px', marginTop: '10px' }}>
                  <div style={{ fontSize: '14px', fontWeight: 700, color: 'var(--color-primary-teal)' }}>
                    UGX {med.selling_price.toLocaleString()}
                  </div>
                  <div style={{ fontSize: '11px', fontWeight: 600, color: isOut ? '#DC2626' : 'var(--color-text-muted)' }}>
                    {isOut ? 'Out of Stock' : `Stock: ${med.current_stock}`}
                  </div>
                </div>
              </div>
            );
      ...
      <div style={{ backgroundColor: '#fff', borderRadius: '16px', border: '1px solid var(--color-border-subtle)', display: 'flex', flexDirection: 'column', height: '100%', boxShadow: '0 4px 16px rgba(0,0,0,0.03)' }}>
        {/* Cart Header */}
        <div style={{ padding: '20px', borderBottom: '1px solid var(--color-border-subtle)', display: 'flex', alignItems: 'center', gap: '10px' }}>
          <ShoppingCart size={20} color="var(--color-primary-teal)" />
        ...
        {/* FEFO Notice */}
        <div style={{ padding: '10px 16px', backgroundColor: '#F0FDF9', borderBottom: '1px solid #CCFBF1', fontSize: '11px', color: '#0F766E', display: 'flex', alignItems: 'center', gap: '6px' }}>
          <CheckCircle size={14} />
          <span>FEFO Active: Stock deducted from earliest expiry batch.</span>
        </div>

        {error && (
          <div style={{ padding: '10px 16px', backgroundColor: '#FEE2E2', color: '#DC2626', fontSize: '12px' }}>
            {error}
          </div>
        )}
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px' }}>
          {cart.length === 0 ? (
            <div style={{ textAlign: 'center', color: 'var(--color-text-muted)', marginTop: '40px', fontSize: '13px' }}>
              Cart is empty. Click medicine cards to add to checkout.
            </div>
          ) : (
            cart.map(item => (
              <div key={item.medicine.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 0', borderBottom: '1px solid var(--color-border-subtle)' }}>
                <div style={{ flex: 1, paddingRight: '8px' }}>
                  <div style={{ fontSize: '13px', fontWeight: 600, color: 'var(--color-charcoal-navy)' }}>{item.medicine.name}</div>
                  <div style={{ fontSize: '11px', color: 'var(--color-text-muted)' }}>UGX {item.medicine.selling_price.toLocaleString()} each</div>
                </div>

                <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                  <button onClick={() => handleUpdateQty(item.medicine.id, -1)} style={{ padding: '4px', borderRadius: '4px', backgroundColor: '#F1F5F9' }}><Minus size={14} /></button>
                  <span style={{ fontSize: '13px', fontWeight: 600, width: '24px', textAlign: 'center' }}>{item.quantity}</span>
                  <button onClick={() => handleUpdateQty(item.medicine.id, 1)} style={{ padding: '4px', borderRadius: '4px', backgroundColor: '#F1F5F9' }}><Plus size={14} /></button>
